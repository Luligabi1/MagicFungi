...
public class MorbusHeartItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if(!world.getGameRules().getBoolean(GameRuleRegistry.DO_MORBUS_SPREADING) && MagicFungi.CONFIG.canUseMorbusCorruptionItems) {
            ...
            user.sendMessage(Text.translatable("message.magicfungi.heart_of_morbus").formatted(Formatting.GRAY, Formatting.ITALIC), false);
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(Text.translatable("tooltip.magicfungi.heart_of_morbus.1").formatted(Formatting.GRAY, Formatting.ITALIC));
        tooltip.add(Text.translatable("tooltip.magicfungi.heart_of_morbus.2").formatted(Formatting.GRAY, Formatting.ITALIC));
    }
}